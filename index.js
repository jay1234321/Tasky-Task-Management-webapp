const openTaskContainer = document.querySelector(".task__modal__body");
...
<div class="card">
  ...
  <div class="card-footer">
    <button class="btn btn-outline-primary" name=${taskData.id} data-bs-toggle = "modal" data-bs-target="#CardPopUp" onclick="openTask.apply(this,arguments)">Open Task</button>
  </div>
</div>
</div>`;
};
...
const openTask = (event) => {
  ...
  taskTitle = parentElement.childNodes[3].childNodes[3].textContent;
  taskDescription = parentElement.childNodes[3].childNodes[5].textContent;
  taskType = parentElement.childNodes[3].childNodes[7].textContent;
  console.log(targetID,taskImage,taskTitle,taskDescription);


  const modalHTML = `
  <div id=${targetID}>
  <img src=${taskImage} alt="bg image" class="img-fluid place__holder__image mb-3">
  <strong class="text-sm text-muted">${taskTitle}</strong>
  <h2 class="my-3">${taskType}</h2>
  <p class="lead">
    ${taskDescription}
  </p>
  </div>`;

  openTaskContainer.innerHTML = modalHTML;
